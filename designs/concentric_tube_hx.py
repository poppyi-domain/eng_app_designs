...
class Hx:
    def __init__(self, annulus_fluid_data: dict, pipe_fluid_data: dict, k_wall: float,
                 t_annulus_cold_kelvin: float, t_annulus_hot_kelvin: float,
                 t_pipe_hot_kelvin: float, t_pipe_cold_kelvin: float,
                 dia_annulus_inner_m: float, dia_pipe_outer_m: float, dia_pipe_inner_m: float,
                 mass_flow_annulus: float, mass_flow_pipe: float,
                 temperature_configuration_type: str, flow_configurate_type: str):
    ...
    def _get_cp_fluid(self, fluid_temp_mean_kelvin, fluid_type):
        ...
        if np.max(fluid_temp_mean_kelvin) > max(temp):
            raise Warning(
                'Temperature exceeds Cp data maximum temperature value of {} Kelvin'.format(
                    max(temp)))
        elif np.min(fluid_temp_mean_kelvin) < min(temp):
            raise Warning('Temperature below Cp data minimum value of {} Kelvin'.format(
                min(temp)))
        else:
    ...
    def nusselt_pipe(self):
        ...
        if self.reynolds_pipe() < 2300:
            ...
            if pr < 0.7 or pr > 160:
                raise Warning(
                    'Inner pipe Prandtl number is {:.4f}, which is out of range: 0.7 < Pr < 160'
                    .format(self.prandtl_pipe))
            return 0.023 * self.reynolds_pipe()**0.8 * self.prandtl_pipe**0.4
        else:
            raise Warning(
                'Average Reynolds number of fluid in pipe calculated as {:.2f} which is in transitional regime'
    ...
    def nusselt_annulus(self):
        ...
        if re < 2300:
            ...
            if self.prandtl_annulus < 0.7 or self.prandtl_annulus > 160:
                raise Warning(
                    'Annulus Prandtl number is {:.4f}, which is out of range: 0.7 < Pr < 160'.
                    format(self.prandtl_annulus))
            return 0.023 * re**0.8 * self.prandtl_annulus**0.4
        else:
            raise Warning(
                'Average Annulus fluid Reynolds number calculated as {:.2f} which is in transitional regime'
                ' results unreliable. Acceptable range is: 2300 > Reynolds > 10000'.format(re))
    ...
    def u_overall(self):
        ...
        return 1 / (
            self._dia_pipe_outer_m /
            (self._dia_pipe_inner_m * self.h_pipe) + 1 / self.h_annulus +
    ...
    def check_pipe_temperatures_initial(self):
        if self.t_annulus_hot < self.t_annulus_cold:
            raise Warning(
                'Annulus hot temperature must be greater than annulus cold temperature')
    ...
    def check_pipe_temperatures_final(self):
        ...
        if (self.t_pipe_hot > self.t_annulus_hot) != (self.t_pipe_cold > self.t_annulus_cold):
            raise Warning(
                'Error! Heat exchanged from cold to hot! Check fluid inlet temperatures and flow rates.'
            )

    def calculate_inlet_outlet_temperatures(self):
        if self._configuration_type == "fluid annulus hot":
            self.q_exchanged = self._mass_flow_pipe * self.cp_pipe * (self.t_pipe_hot -
                                                                      self.t_pipe_cold)
            self.t_annulus_hot = self.q_exchanged / (self._mass_flow_annulus *
                                                     self.cp_annulus) + self.t_annulus_cold
            ...
            self.q_exchanged = self._mass_flow_pipe * self.cp_pipe * (self.t_pipe_hot -
                                                                      self.t_pipe_cold)
            self.t_annulus_cold = self.t_annulus_hot - self.q_exchanged / (
                self._mass_flow_annulus * self.cp_annulus)
            self.t_annulus_hot = self.t_annulus_hot
            self.t_pipe_hot = self.t_pipe_hot
            self.t_pipe_cold = self.t_pipe_cold

        elif self._configuration_type == "fluid pipe hot":
            self.q_exchanged = self._mass_flow_annulus * self.cp_annulus * (
                self.t_annulus_hot - self.t_annulus_cold)
            self.t_pipe_hot = self.q_exchanged / (self._mass_flow_pipe *
                                                  self.cp_pipe) + self.t_pipe_cold
            self.t_pipe_cold = self.t_pipe_cold
            self.t_annulus_hot = self.t_annulus_hot
            self.t_annulus_cold = self.t_annulus_cold
            ...
            self.q_exchanged = self._mass_flow_annulus * self.cp_annulus * (
                self.t_annulus_hot - self.t_annulus_cold)
            self.t_pipe_cold = self.t_pipe_hot - self.q_exchanged / (self._mass_flow_pipe *
                                                                     self.cp_pipe)
    ...
    def lmtd(self):
        ...
        if self._flow_configuration_type == 'counter-flow':
            ...
            raise Warning(
                'Unrecognised flow configuration should be either counter-flow or parallel-flow'
            )
    ...
    def reynolds_annulus(self):
        return 4 * self._mass_flow_annulus / (
            np.pi * (self._dia_annulus_inner_m + self._dia_pipe_outer_m) * self.visc_annulus)

    def _get_laminal_flow_nusselt_annulus(self):
        di_do = self._dia_pipe_outer_m / self._dia_annulus_inner_m
        if di_do < 0.05:
            raise Warning(
                'Ratio of diameter of inner pipe to annulus is too small and not supported')
        return np.interp(di_do, [0, 0.05, 0.10, 0.25, 0.50, 1.00],
...
def main(form_data):
    numerics = [
        "fluid_pipe_hot_temp_c", "fluid_pipe_cold_temp_c", "fluid_pipe_flow_rate_kg_s",
        "fluid_annulus_hot_temp_c", "fluid_annulus_cold_temp_c",
        "fluid_annulus_flow_rate_kg_s", "dia_inner_annulus_mm", "dia_outer_pipe_mm",
        "dia_inner_pipe_mm", "thermal_cond_pipe_w_m_k"
    ]
    ...
    return {
        ...
        "fluid_pipe_hot_temp_c_out": {
            "action": "update",
            "value": heat_ex.t_pipe_hot - 273.15
        },
        "fluid_pipe_cold_temp_c_out": {
            "action": "update",
            "value": heat_ex.t_pipe_cold - 273.15
        },
        "fluid_annulus_hot_temp_c_out": {
            "action": "update",
            "value": heat_ex.t_annulus_hot - 273.15
        },
        "fluid_annulus_cold_temp_c_out": {
            "action": "update",
            "value": heat_ex.t_annulus_cold - 273.15
        },
        "log_mean_temp": {
            "action": "update",
            "value": heat_ex.lmtd
        },
        "heat_exchanger_length": {
            "action": "update",
            "value": heat_ex.heat_exchanger_length
        },
        "mean_temp_annulus": {
            "action": "update",
            "value": heat_ex.annulus_mean_temp - 273.15
        },
        "mean_temp_pipe": {
            "action": "update",
            "value": heat_ex.pipe_mean_temp - 273.15
        },
        "reynolds_annulus": {
            "action": "update",
            "value": heat_ex.reynolds_annulus()
        },
        "reynolds_pipe": {
            "action": "update",
            "value": heat_ex.reynolds_pipe()
        },
        "prandtl_pipe": {
            "action": "update",
            "value": heat_ex.prandtl_pipe
        },
        "prandtl_annulus": {
            "action": "update",
            "value": heat_ex.prandtl_annulus
        },
        # "annulus_hydraulic_diameter_mm": {"action": "update", "value": hydraulic_dia_annulus_meters*1e3},
        "nusselt_pipe": {
            "action": "update",
            "value": heat_ex.nusselt_pipe
        },
        "nusselt_annulus": {
            "action": "update",
            "value": heat_ex.nusselt_annulus
        },
        "heat_trans_coeff_annulus": {
            "action": "update",
            "value": heat_ex.h_annulus
        },
        "heat_trans_coeff_pipe": {
            "action": "update",
            "value": heat_ex.h_pipe
        },
        "overall_heat_transfer_coeff": {
            "action": "update",
            "value": heat_ex.u_overall
        },
    }
